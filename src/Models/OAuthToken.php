...
class OAuthToken implements \JsonSerializable
{
    ...
    {
        $serializer->addAsSubelement($element, 'access_token', $this->accessToken);
        $serializer->addAsSubelement($element, 'token_type', $this->tokenType);
        $serializer->addAsSubelement($element, 'expires_in', $this->expiresIn);
        $serializer->addAsSubelement($element, 'scope', $this->scope);
        $serializer->addAsSubelement($element, 'expiry', $this->expiry);
        $serializer->addAsSubelement($element, 'refresh_token', $this->refreshToken);
    }
    ...
    {
        $accessToken = $serializer->fromElement($element, 'access_token', 'string');
        $tokenType = $serializer->fromElement($element, 'token_type', 'string');

        $instance = new self($accessToken, $tokenType);

        $expiresIn = $serializer->fromElement($element, 'expires_in', '?int');
        $instance->setExpiresIn($expiresIn);

        $scope = $serializer->fromElement($element, 'scope', '?string');
        $instance->setScope($scope);

        $expiry = $serializer->fromElement($element, 'expiry', '?int');
        $instance->setExpiry($expiry);

        $refreshToken = $serializer->fromElement($element, 'refresh_token', '?string');
